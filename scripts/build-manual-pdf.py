...
The version printed on the cover and page header comes from MANUAL_VERSION when
set (the CI passes the release tag), otherwise from package.json. The CI builds
from the release commit, where ci-bump-release.sh already bumped package.json, so
both sources agree and the Markdown never needs a version stamped into it.
"""
...
OUTPUT_PATH = OUTPUT_DIRECTORY / "manual-personal-taefoto.pdf"
...
def build_html(source: str, stylesheet: str, version: str) -> str:
    ...
    subtitle = html.escape(metadata.get("subtitle", "Préstamo de Equipos — TAE Foto"))
...
<body>
  ...
  <div id="page-header">Manual del personal &middot; Préstamo de Equipos TAE Foto &middot; {safe_version}</div>
...
def main() -> None:
    ...
    version = os.environ.get("MANUAL_VERSION") or f"v{package['version']}"
