...
//
// The Worker URL ships inside the app binary, so anyone who unpacks it can POST
// here directly. Per-IP rate limiting mitigates spam; it does not eliminate it.
// If it ever gets abused, the next step is a shared secret or signed requests.

const REPO = "Leoglez10/Tae-Foto-P15";
...
const oneLine = (value) => String(value ?? "").replace(/\s+/g, " ").trim().slice(0, METADATA_MAX);
...
export default {
  async fetch(request, env) {
    ...
    if (!titulo || titulo.length > TITULO_MAX) return json(400, { error: "Título inválido" });
    if (!descripcion || descripcion.length > DESCRIPCION_MAX) return json(400, { error: "Descripción inválida" });
    if (String(payload?.version ?? "").length > METADATA_MAX || String(payload?.so ?? "").length > METADATA_MAX) {
    ...
    const response = await fetch(`https://api.github.com/repos/${REPO}/issues`, {
      ...
      headers: {
        ...
        "User-Agent": "taefoto-p15-feedback",
