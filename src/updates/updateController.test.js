...
import { createUpdateController, UPDATE_INTERVAL_MS } from "./updateController.js";
...
async function doesNotReject(promise) {
  await assert.doesNotReject(async () => promise);
}
...
test("no update is current; browser and unpublished/debug builds never call updater", async () => {
  ...
  for (const readiness of ["unsupported", "development"]) {
  ...
  f.deps.readiness = async () => {
    throw new Error("no native calls in browser");
  };
...
test("setup deduplicates startup, bounds cadence and cleans timer/resources", async () => {
  const f = fixture();
  let timer;
  let activeTimers = 0;
...
for (const failure of ["signature mismatch", "download interrupted", "installer failed"]) {
  test(`${failure} is retryable only with renewed consent`, async () => {
    const f = fixture();
    f.update.downloadAndInstall = async () => {
      f.calls.download++;
...
test("only consent records the pending update, and a broken recorder cannot stop it", async () => {
  const f = fixture();
  const consents = [];
  f.deps.onInstallConsent = (version, notes) => {
    consents.push([version, notes]);
  };
  const controller = createUpdateController(f.deps);
  await controller.check();
  await controller.install(async () => false);
...
test("controller APIs swallow dependency and listener failures", async () => {
  const controller = createUpdateController({
    isDesktop: () => {
      throw new Error("desktop probe failed");
    },
    readiness: async () => {
  ...
  await doesNotReject(controller.check(true));
  assert.equal(controller.getSnapshot().status, "error");
  assert.equal(controller.getSnapshot().error, "check");
  assert.doesNotThrow(() => controller.defer());
});
